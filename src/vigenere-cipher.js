...
class VigenereCipheringMachine {
  ...
  encrypt(wordToEncrypt, keyToEncrypt) {
    ...
    for(let j = 0; j < wordToNum.length; j++) {
      if(typeof wordToNum[j] === 'number') {
        encoded.push(wordToNum[j] + keyToNum[count]>= 26 
          ? (wordToNum[j]+ keyToNum[count]) - 26 
          : wordToNum[j] + keyToNum[count])
        count++
      } else {
    ...
    for(let e = 0; e < encoded.length; e++) {
      typeof encoded[e] === 'number' 
      ? result.push(letters[encoded[e]]) 
      : result.push(encoded[e])
    }
  ...
  decrypt(wordToDecrypt, ketToDecrypt) {
    ...
    for(let j = 0; j < wordToNum.length; j++) {
      if(typeof wordToNum[j] === 'number') {
        encoded.push(wordToNum[j]- keyToNum[count]< 0 
          ? (wordToNum[j] + 26 - keyToNum[count]) 
          : wordToNum[j] - keyToNum[count])
        count++
      } else {
        encoded.push(wordToNum[j])
